...
# Автоматический ремонт venv и triton при каждом запуске start.py.
# Гарантирует, что после рестарта Kaggle вернулись биты +x на интерпретатор
# и triton.ptxas. Если ремонт не поможет — включает TRITON_INTERPRET_MODE.
# verbose=False скрывает лишние логи (выводит только критичные).
ke.install_python(verbose=False)
